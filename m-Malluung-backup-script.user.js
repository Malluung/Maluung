        ...
        generateHTML(data, colorSettings) {
            ...
            function renderMessages(messages) {
                ...
                for (const msg of messages) {
                    ...
                    if (msg.alternatives) {
                        ...
                        if (visibleAlts.length > 0) {
                            radioGroupCounter++;
                            const radioGroupName = `radio-group-${radioGroupCounter}`;
                            ...
                            // 라디오 버튼 HTML 생성
                            const radiosHtml = visibleAlts.map((alt, altIndex) => {
                                const radioId = `radio-${radioGroupCounter}-${altIndex}`;
                                return `<input type="radio" name="${radioGroupName}" id="${radioId}" class="alt-radio-input" ${altIndex === 0 ? 'checked' : ''}>`;
                            }).join('');
                            ...
                            const contentsHtml = visibleAlts.map((alt, altIndex) => {
                                ...
                                return `<div class="answer-content" data-radio-id="${radioId}" style="--ai-bubble-color: ${altModelColor};"><div class="author-header"><span class="author-name">${alt.author}</span><span class="message-meta">${utils.formatDate(new Date(alt.timestamp))} • ${alt.model || 'N/A'}</span></div><div class="message-content" style="background-color: ${altModelColor}; color: #000;">${processContent(alt.content)}</div></div>`;
                            }).join('');

                            // 라벨 버튼 HTML 생성
                            const labelsHtml = visibleAlts.map((alt, altIndex) => {
                                const radioId = `radio-${radioGroupCounter}-${altIndex}`;
                                const altModelColor = getModelColor(alt.model);
                                // 각 버튼에 대한 CSS 규칙 추가
                                interactiveCss += `#${radioId}:checked ~ .interactive-message-body .answer-content[data-radio-id="${radioId}"] { display: block; }\n`;
                                interactiveCss += `#${radioId}:checked ~ .interactive-message-body .alt-controls label[for="${radioId}"] { background-color: ${altModelColor}; border-color: ${altModelColor}; }\n`;
                                return `<label for="${radioId}" class="alt-btn-label">${altIndex + 1}</label>`;
                            }).join('');

                            messageBlock = `
                                <div class="message ai interactive-container">
                                    ${radiosHtml}
                                    ${firstAltAvatar}
                                    <div class="interactive-message-body">
                                        ${contentsHtml}
                                        <div class="alt-controls"><b>답변 선택: </b>${labelsHtml}</div>
                                    </div>
                                </div>`;
            ...
            return `<!DOCTYPE html><html lang="ko"><head><meta charset="UTF-8"><title>${data.characterName} - 채팅 백업</title><style>
                  * { box-sizing: border-box; }
                  ...
                  h1 { margin: 0; color: #fff; } .meta-info { font-size: 0.9em; color: var(--meta-text-color); }
                  .message { display: flex; margin-bottom: 20px; align-items: flex-start; } .message.user { justify-content: flex-end; }
                  .avatar { width: 36px; height: 36px; border-radius: 50%; object-fit: cover; flex-shrink: 0; margin-top: 5px; background-color: #333; }
                  .message.ai .avatar { margin-right: 12px; } .message.user .avatar { display: none; }
                  .message-body, .interactive-message-body { max-width: 85%; display: flex; flex-direction: column; width: 100%; }
                  .author-header { display: flex; align-items: center; margin-bottom: 5px; flex-wrap: wrap; padding: 0 10px; } .author-header.user { justify-content: flex-end; }
                  .author-name { font-weight: 600; font-size: 0.9em; color: var(--author-name-color); } .message.user .author-name { display: none; }
                  ...
                  .answer-content { display: none; animation: fadeIn 0.3s; }
                  .answer-content .message-content { width: 100%; } @keyframes fadeIn { from { opacity: 0; } to { opacity: 1; } }
                  ...
                  .alt-controls { margin-top: 10px; padding: 10px 12px; border-radius: 20px; background: #2a2a2a; }

                  /* [수정] CSS 기반 버튼 시스템 */
                  .interactive-container { position: relative; }
                  .alt-radio-input { display: none; }
                  .alt-btn-label { background-color: #444; color: white; border: 1px solid #555; border-radius: 15px; padding: 5px 10px; cursor: pointer; font-size: 12px; margin-right: 5px; transition: all 0.2s; -webkit-user-select: none; user-select: none;}
                  .alt-btn-label:hover { opacity: 0.8; }

                  /* [수정] 자바스크립트 대신 동적으로 생성될 CSS 규칙 */
                  ${interactiveCss}

                 </style></head><body><div class="container"><div class="header"><h1>${data.characterName}</h1><p class="meta-info">백업 일시: ${utils.formatDate(new Date(data.extractedAt))} / 총 대화 턴 수: ${visibleMessageCount}</p></div>${messagesHtml}</div></body></html>`;
        }
    }
