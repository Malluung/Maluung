// ==UserScript==
// @name         크랙 채팅 백업 모바일
// @namespace    https://github.com/Malluung
// @version      1.0
    ...
    class BackupUI {
        ...
        async addBackupButton() {
            ...
            try {
                ...
                const handlePressStart = (e) => {
                    ...
                    this.isLongPress = false;
                    fab.style.transition = 'all 0.1s ease-in-out';
                    fab.style.transform = 'scale(0.9)';

                    this.longPressTimer = setTimeout(() => {
                        this.isLongPress = true;
                        fab.innerHTML = '⚙️';
                        fab.style.background = 'linear-gradient(45deg, #888, #555)';
                ...
                fab.addEventListener('mousedown', handlePressStart);
                fab.addEventListener('mouseup', handlePressEnd);
                fab.addEventListener('mouseleave', handlePressCancel);
                fab.addEventListener('touchstart', (e) => { e.preventDefault(); handlePressStart(e); }, { passive: false });
                fab.addEventListener('touchend', (e) => { e.preventDefault(); handlePressEnd(e); });
                fab.addEventListener('touchcancel', handlePressCancel);

                document.body.appendChild(fab);
            } catch (error) {
                console.error("백업 버튼 추가 실패:", error);
                const fab = document.createElement('div');
                fab.id = 'oneClickBackupButton';
                fab.style.cssText = `position:fixed; bottom: 25px; right: 25px; /* ... */`;
                document.body.appendChild(fab);
            }
        }
        ...
        async runBackup(force = false) {
            ...
            try {
                ...
                if (force) {
                    ...
                    const mergedMessages = [...currentChatData.messages];
                    const currentIds = new Set(mergedMessages.flatMap(msg => msg.alternatives ? [msg._id, ...msg.alternatives.map(a => a._id)] : [msg._id]));

                    if (savedData.messages) {
                        const allSavedMsgs = savedData.messages.flatMap(msg => msg.alternatives ? [msg, ...msg.alternatives] : [msg]);
                        for (const savedMsg of allSavedMsgs) {
                            if (!currentIds.has(savedMsg._id)) {
                                const parentTurnId = savedMsg.parentTurnId;
                                let targetContainer = mergedMessages.find(m => m.alternatives && m.alternatives.some(a => a.parentTurnId === parentTurnId));

                                if (targetContainer) {
                                    if (!targetContainer.alternatives.some(a => a._id === savedMsg._id)) {
                                        targetContainer.alternatives.push(savedMsg);
                                    }
                                } else {
                                    mergedMessages.push(savedMsg);
                                }
                            }
                        }
                    }
                    ...
                    mergedMessages.forEach(msg => {
                        if (msg.alternatives) {
                            msg.alternatives.sort((a,b) => new Date(a.timestamp) - new Date(b.timestamp));
                        }
                    });
                    ...
                    mergeDeletedStatus(finalData.messages);

                    const stringifyForCompare = (data) => JSON.stringify(data, (key, value) => key === 'extractedAt' ? undefined : value);
                    const hasChanges = stringifyForCompare(savedData) !== stringifyForCompare(finalData);
        ...
        generateHTML(data, colorSettings) {
                ...
                for (const msg of messages) {
                    ...
                    if (msg.alternatives) {
                        ...
                        if (visibleAlts.length > 0) {
                            ...
                            const radioGroupName = `alt-group-${msg._id}`;

                            const contentsHtml = visibleAlts.map((alt, altIndex) => {
                                const radioId = `alt-radio-${alt._id}`;
                                const altModelColor = getModelColor(alt.model);

                                return `
                                    <div class="alt-container" id="container-for-${alt._id}">
                                        <input type="radio" name="${radioGroupName}" id="${radioId}" class="alt-radio-input" ${altIndex === 0 ? 'checked' : ''}>
                                        <label for="${radioId}" class="alt-btn" style="--active-bg: ${altModelColor}; --active-border: ${altModelColor};">${altIndex + 1}</label>
                                        <div class="answer-content-wrapper">
                                            <div class="answer-content">
                                                <div class="author-header">
                                                    <span class="author-name">${alt.author}</span>
                                                    <span class="message-meta">${utils.formatDate(new Date(alt.timestamp))} • ${alt.model || 'N/A'}</span>
                                                </div>
                                                <div class="message-content" style="background-color: ${altModelColor}; color: #000;">${processContent(alt.content)}</div>
                                            </div>
                                        </div>
                                    </div>
                                `;
                            ...
                            const contentHtml = `<div class="interactive-message">${contentsHtml}</div>`;
                            messageBlock = `<div class="message ai">${firstAltAvatar}<div class="message-body">${contentHtml}</div></div>`;
            ...
            const visibleMessageCount = data.messages ? data.messages.reduce((count, msg) => {
                ...
                if (msg.alternatives) {
                    return count + (msg.alternatives.filter(a => !a.isDeleted).length > 0 ? 1 : 0);
                }
                return count + 1;
            }, 0) : 0;

            return `<!DOCTYPE html><html lang="ko"><head><meta charset="UTF-8"><title>${data.characterName} - 채팅 백업</title>
                <style>
                    * { box-sizing: border-box; }
                    :root { --bg-color: ${colorSettings.backgroundColor}; --text-color: ${colorSettings.textColor}; --user-bubble-color: ${colorSettings.userBubble}; --author-name-color: ${colorSettings.authorNameColor}; --meta-text-color: ${colorSettings.metaTextColor}; }
                    ...
                    .header { text-align: center; border-bottom: 1px solid #444; padding-bottom: 20px; margin-bottom: 30px; }
                    h1 { margin: 0; color: #fff; }
                    .meta-info { font-size: 0.9em; color: var(--meta-text-color); }
                    .message { display: flex; margin-bottom: 20px; align-items: flex-start; }
                    .message.user { justify-content: flex-end; }
                    .avatar { width: 36px; height: 36px; border-radius: 50%; object-fit: cover; flex-shrink: 0; margin-top: 5px; background-color: #333; }
                    .message.ai .avatar { margin-right: 12px; }
                    .message.user .avatar { display: none; }
                    .message-body { max-width: 85%; display: flex; flex-direction: column; }
                    .author-header { display: flex; align-items: center; margin-bottom: 5px; flex-wrap: wrap; padding: 0 10px; }
                    .author-header.user { justify-content: flex-end; }
                    .author-name { font-weight: 600; font-size: 0.9em; color: var(--author-name-color); }
                    .message.user .author-name { display: none; }
                    .message-meta { font-size: 0.75em; color: var(--meta-text-color); margin: 0 8px; }
                    .message-content { padding: 12px 18px; border-radius: 20px; line-height: 1.6; word-break: break-word; }
                    .content-text p { margin: 0 0 1em 0; } .content-text p:last-child { margin-bottom: 0; }
                    .message.user .message-body .message-content { background-color: var(--user-bubble-color); }
                    .content-text .chat-image { width: 100%; height: auto; max-height: 400px; object-fit: contain; border-radius: 8px; margin-top: 8px; display: block; margin-left: auto; margin-right: auto; }

                    /* --- 라디오 버튼 방식 CSS --- */
                    .interactive-message .alt-container { margin-bottom: 10px; }
                    .interactive-message .alt-container:last-child { margin-bottom: 0; }
                    .alt-radio-input { position: absolute; opacity: 0; width: 0; height: 0; }
                    .alt-btn { display: inline-block; background-color: #444; color: white; border: 1px solid #555; border-radius: 15px; padding: 5px 10px; cursor: pointer; font-size: 12px; margin-bottom: 8px; transition: all 0.2s; }
                    .answer-content-wrapper { display: none; animation: fadeIn 0.3s; }
                    .alt-radio-input:checked + .alt-btn {
                        background-color: var(--active-bg, #8c9eff);
                        border-color: var(--active-border, #8c9eff);
                    }
                    .alt-radio-input:checked + .alt-btn + .answer-content-wrapper {
                        display: block;
                    }
                    @keyframes fadeIn { from { opacity: 0; } to { opacity: 1; } }
                 </style>
                </head><body><div class="container"><div class="header"><h1>${data.characterName}</h1><p class="meta-info">백업 일시: ${utils.formatDate(new Date(data.extractedAt))} / 총 대화 턴 수: ${visibleMessageCount}</p></div>${messagesHtml}</div>
                <script>
                        ...
                        if (e.target.classList.contains('alt-btn')) {
                            requestAnimationFrame(function() {
                                ...
                                if (radio && radio.checked) {
                                    const interactiveMessageContainer = e.target.closest('.interactive-message');
                                    if (interactiveMessageContainer) {
                                        const allButtonsInGroup = interactiveMessageContainer.querySelectorAll('.alt-btn');
                                        if (allButtonsInGroup.length > 0) {
                                            const lastButton = allButtonsInGroup[allButtonsInGroup.length - 1];
                                            const rect = lastButton.getBoundingClientRect();
                                            if (rect.bottom > window.innerHeight) {
                                                lastButton.scrollIntoView({ behavior: 'smooth', block: 'end', inline: 'nearest' });
                                            }
                                        }
                                    }
                                }
